...
public class MigrationResult<T> {
    ...
    public MigrationStatus getFinalStatus() {
        return containsMigrationStatus(MigrationStatus.ERROR) ? MigrationStatus.ERROR
                : (containsMigrationStatus(MigrationStatus.WARNING) ? MigrationStatus.WARNING
                        : MigrationStatus.SUCCESS);
    }

    private boolean containsMigrationStatus(MigrationStatus migrationStatus) {
        List<MigrationStepReport> list = migrationStepReportList.stream()
                .filter(stepReport -> migrationStatus.getValue().equals(stepReport.getMigrationStatus().getValue()))
                .collect(Collectors.toList());
        return list.size() > 0;
    }
    ...
    private List<MigrationStatus> getStatusList() {
        switch (getFinalStatus()) {
            ...
            default:
                return Arrays.asList(MigrationStatus.SUCCESS, MigrationStatus.ERROR, MigrationStatus.WARNING,
