...
public class LocalizationFactory implements PageFactory {
    ...
    private <P extends Previewable & Identifiable> Object getLocalization(P previewable) {
        if (previewable instanceof Page) {
            try {
                var localizationPath = pageRepository.resolvePath(previewable.getId())
                        .resolve("assets/json/localization.json");
                ...
                logger.error("An error occurred while loading assets/json/localization.json for page <{}>",
                        previewable.getId(), e);
                return "{}";
            }
        } else {
