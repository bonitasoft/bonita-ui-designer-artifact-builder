...
class AssetVisitorTest {
    ...
    void should_return_list_of_asset_needed_by_widgets_in_container_who_have_an_asset() throws Exception {
        ...
        when(widgetRepository.get("pbContainer")).thenReturn(
                aWidget().assets(anAsset().withName("container.min.js").withType(AssetType.JAVASCRIPT)).build());
        Set<Asset> assets = assetVisitor.visit(aContainer().with(component1, component2).build());
    ...
    void should_return_list_of_asset_needed_by_widgets_in_formcontainer() throws Exception {
        ...
        Component component2 = mockComponentFor(aWidget(), UUID.randomUUID().toString(),
                anAsset().withName("myfile.css").withType(AssetType.CSS));
    ...
    void should_return_list_of_asset_needed_by_widgets_in_formcontainer_who_have_an_asset() throws Exception {
        ...
        Component component2 = mockComponentFor(aWidget(), UUID.randomUUID().toString(),
                anAsset().withName("myfile.css").withType(AssetType.CSS));
        when(widgetRepository.get("pbFormContainer")).thenReturn(
                aWidget().assets(anAsset().withName("formContainer.min.js").withType(AssetType.JAVASCRIPT)).build());
        when(widgetRepository.get("pbContainer")).thenReturn(aWidget().build());
        Set<Asset> assets = assetVisitor.visit(aFormContainer().with(
                aContainer().with(component1, component2)).build());
    ...
    void should_return_list_of_asset_needed_by_widgets_in_tabscontainer_plus_uibootstrap_which_is_needed_by_tabscontainer()
        ...
        Component component2 = mockComponentFor(aWidget(), UUID.randomUUID().toString(),
                anAsset().withName("myfile.css").withType(AssetType.CSS));
        when(widgetRepository.get("pbTabsContainer")).thenReturn(
                aWidget().assets(anAsset().withName("bootstrap.min.js").withType(AssetType.JAVASCRIPT)).build());
        when(widgetRepository.get("pbContainer")).thenReturn(aWidget().build());
    ...
    void should_return_list_of_asset_needed_by_widgets_in_modal_container() throws Exception {
        ...
        when(widgetRepository.get("pbContainer")).thenReturn(aWidget().build());
        Set<Asset> assets = assetVisitor
                .visit(aModalContainer().with(aContainer().with(component1, component2)).build());
    ...
    void should_good_list_widget_asset__if_one_is_inactive() throws Exception {
        ...
        assertThat(assets).extracting("name").contains("myfile.js", "myfile.css", "myfileBis.js",
                "http://mycdn.com/myfile.js");
    ...
    private Component mockComponentFor(WidgetBuilder widgetBuilder, String id, AssetBuilder... assetBuilders)
            throws Exception {
