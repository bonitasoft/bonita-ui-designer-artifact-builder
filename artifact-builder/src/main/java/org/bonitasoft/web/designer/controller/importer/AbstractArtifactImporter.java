...
public abstract class AbstractArtifactImporter<T extends Identifiable> {
    ...
    protected AbstractArtifactImporter(JsonHandler jsonHandler, ArtifactService<T> artifactService,
            Repository<T> repository, DependencyImporter<?>... dependencyImporters) {
    ...
    public ImportReport tryToImportAndGenerateReport(Import anImport, boolean ignoreConflicts) {
        ...
        try {
            ...
            var importException = new ImportException(JSON_STRUCTURE,
                    format("Could not read json file [%s]", modelFile), e);
            importException.addInfo("modelfile", modelFile);
            throw importException;
        }
    }

    private void saveComponent(Path resources, T element, Map<DependencyImporter, List<?>> dependencies)
            throws IOException {
        ...
        if (!StringUtils.isEmpty(elementUUID)) {
            ...
            try {
                ...
                logger.info("Imported artifact {} does not have an uuid attribute but its id has the UUID format."
                        + "The migration will give it an uuid attribute with the same value as its id.",
                        element.getId());
                ...
                logger.info(
                        format("Imported artifact %s does not have an uuid attribute and its id does not have the UUID format."
                                + "A new UUID will be generated by the migration so it will not replace another artifact.",
                                element.getId()));
            }
        }
