...
public class DefaultPageService extends AbstractAssetableArtifactService<PageRepository, Page> implements PageService {
    ...
    public DefaultPageService(PageRepository pageRepository, PageMigrationApplyer pageMigrationApplyer,
            ComponentVisitor componentVisitor,
            AssetVisitor assetVisitor,
            UiDesignerProperties uiDesignerProperties, AssetService<Page> pageAssetService,
            WebResourcesVisitor webResourcesVisitor) {
    ...
    public Page save(String pageId, Page page) {
        try {
            ...
            if (!existingPage.isCompatible()) {
                throw new IncompatibleException("Page " + existingPage.getId()
                        + " is in an incompatible version. Newer UI Designer version is required.");
            }
        ...
        if (!savedPage.getId().equals(pageId)) {
            assetService.duplicateAsset(repository.resolvePath(pageId), repository.resolvePath(pageId), pageId,
                    savedPage.getId());
    ...
    public List<String> getResources(Page page) {
        ...
        List<String> manual = page.getWebResources().stream().map(WebResource::toDefinition)
    ...
    public MigrationResult<Page> migrateWithReport(Page pageToMigrate) {
        ...
        if (!migratedResult.getFinalStatus().equals(MigrationStatus.ERROR)) {
            ...
            migratedResult.getMigrationStepReportList().stream()
                    .filter(report -> !MigrationStatus.SUCCESS.equals(report.getMigrationStatus()))
                    .forEach(report -> log.error("{}: {} - {}", report.getMigrationStatus(), report.getStepInfo(),
                            report.getComments()));
        }
