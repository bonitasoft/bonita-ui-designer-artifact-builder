...
public class AnyLocalContainerVisitor implements ElementVisitor<Iterable<Element>> {
    ...
    public Iterable<Element> visit(Container container) {
        ...
        return Stream.concat(
                of(container),
                traverse(container.getRows().stream().flatMap(Collection::stream))).collect(Collectors.toList());
    }
    ...
    public Iterable<Element> visit(Previewable previewable) {
        return traverse(
                previewable.getRows().stream().flatMap(Collection::stream)).collect(Collectors.toList());
    }
