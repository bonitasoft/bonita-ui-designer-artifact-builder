...
public class UIBootstrapAssetMigrationStep extends AbstractMigrationStep<Page> {
    ...
    public Optional<MigrationStepReport> migrate(Page page) {
        if (!pageHasAsset(page, "ui-bootstrap") && !widgetHasAsset(page, "ui-bootstrap")) {
            ...
            var msg = format(
                    "Adding %s asset [%s] to [%s] (as it was removed from vendor.min.js). You can remove it if you don't need it.",
                    uiBootstrap.getType(), uiBootstrap.getName(), page.getName());
            logger.info(msg);
            return Optional
                    .of(MigrationStepReport.warningMigrationReport(page.getName(), msg, this.getClass().getName()));
        }
    ...
    public byte[] getContent() {
        try (InputStream is = this.getClass().getResourceAsStream(ASSET_FILE_NAME)) {
            ...
            throw new ArtifactBuilderException(
                    "Missing " + this.getClass().getPackage() + "/" + ASSET_FILE_NAME + " from classpath", e);
        }
    }
}
