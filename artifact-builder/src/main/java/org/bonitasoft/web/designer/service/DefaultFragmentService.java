...
public class DefaultFragmentService extends AbstractArtifactService<FragmentRepository, Fragment>
    ...
    public DefaultFragmentService(FragmentRepository fragmentRepository, PageRepository pageRepository,
            ...
            FragmentIdVisitor fragmentIdVisitor, FragmentChangeVisitor fragmentChangeVisitor,
    ...
    public Fragment rename(Fragment fragment, String name) throws ModelException {
        ...
        checkNameIsUnique(
                ...
                repository.getAll().stream().filter(Predicates.propertyEqualTo("id", fragmentId).negate())
                        .collect(toList()),
                fragment);
    ...
    public Fragment save(String fragmentId, Fragment fragment) throws ModelException {
        checkNameIsUnique(
                ...
                repository.getAll().stream().filter(
                        Predicates.propertyEqualTo("id", fragment.getId()).negate()).collect(toList()),
                fragment);
        ...
        try {
            ...
            var validationErrorStateChanged = currentFragment.getHasValidationError() != fragment
    ...
    private void checkIfFragmentIsCompatible(Fragment fragment) throws ModelException {
        if (fragment.getStatus() != null && !fragment.getStatus().isCompatible()) {
            var message = format("Fragment %s is in an incompatible version. Newer UI Designer version is required.",
                    fragment.getId());
            throw new ModelException(message);
        }
    }

    private void updateReferencesParentArtifacts(Fragment currentFragment, String newFragmentId,
            boolean newHasValidationError) {
    ...
    private <T extends AbstractPage> void updateReference(List<? extends Identifiable> artifacts, String newFragmentId,
            String oldFragmentId, AbstractRepository repo, boolean newHasValidationError) {
    ...
    private void updateArtifactValidationError(AbstractPage artifact, boolean newHasValidationError,
            String newFragmentId) {
