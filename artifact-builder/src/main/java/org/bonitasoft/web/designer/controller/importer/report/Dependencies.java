...
public class Dependencies {
        ...
        for (Map.Entry<DependencyImporter, List<?>> entry : dependenciesAsList.entrySet()) {
            ...
            if (dependencyImporter instanceof ComponentDependencyImporter<?>) {
                dependencies.add((List<Identifiable>) entry.getValue(),
                        (ComponentDependencyImporter<Identifiable>) dependencyImporter);
            }
        }
        return dependencies;
    }
    ...
    private void add(List<Identifiable> identifiables, ComponentDependencyImporter<Identifiable> importer) {
        for (Identifiable identifiable : identifiables) {
            if (importer.exists(identifiable)) {
                addOverwrittenDependency(importer.getComponentName(),
                        importer.getOriginalElementFromRepository(identifiable));
            } else {
    ...
    private Map<String, List<Object>> addDependency(Map<String, List<Object>> map, String componentName,
            Object dependencies) {
        if (map == null) {
            map = new HashMap<>();
        }
