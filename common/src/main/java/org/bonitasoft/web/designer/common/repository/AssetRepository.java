...
public class AssetRepository<T extends Identifiable & Assetable> {
    ...
    protected Path resolveAssetPath(String componentId, Asset asset) {
        ...
        return resolveComponentPath(componentId).resolve(ASSETS).resolve(asset.getType().getPrefix())
                .resolve(asset.getName());
    }
    ...
    protected Path resolveExistingAssetPath(String componentId, Asset asset) {
        ...
        if (!exists(path)) {
            throw new NotFoundException(
                    format("Error when searching asset %s for %s [%s]: asset not found in path '%s'",
     ...
     */
    public List<Asset> findAssetInPathWhithoutComponentId(T component, AssetType type, Path directory)
            throws IOException {
        List<Asset> objects = new ArrayList<>();
    ...
    public void refreshAssets(final T component) {
        var assets = stream(AssetType.values()).map(type -> {
            try {
                var assetTypePath = repository.resolvePath(component.getId())
                        .resolve(Paths.get(ASSETS, type.getPrefix()));
                if (exists(assetTypePath)) {
                    return findAssetInPathWhithoutComponentId(component, type, assetTypePath);
                }
            } catch (IOException e) {
                throw new RepositoryException(format("Failed to initialized assets for %s %s",
                        repository.getComponentName(), component.getId()), e);
            }
