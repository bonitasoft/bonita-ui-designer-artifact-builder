...
public abstract class AbstractRepository<T extends Identifiable> implements Repository<T> {
    ...
    protected AbstractRepository(Path path, JsonFileBasedPersister<T> persister, Loader<T> loader,
            BeanValidator validator, Watcher watcher, Path templatePath) {
        this.path = path;
    ...
    public T save(T component) throws RepositoryException {
        ...
        try {
            ...
            throw new RepositoryException(format("Error while saving %s [%s]", getComponentName(), component.getId()),
                    e);
        }
    }
    ...
    public void createComponentDirectory(T component) {
        try {
            ...
            throw new RepositoryException(format("Impossible to create the folder to save the component [ %s ] : %s",
                    component.getId(), path), ex);
        }
    }
    ...
    private void initializeComponentDirectoryFromTemplate(T component) {
        ...
        if (Files.exists(componentTemplateDir)) {
            try {
                FileUtils.copyDirectory(componentTemplateDir.toFile(), resolvePath(component.getId()).toFile());
            } catch (IOException e) {
                throw new RepositoryException(
                        format("Failed to initialize %s [%s] from template\"", getComponentName(), component.getId()),
                        e);
            }
        }
    }
